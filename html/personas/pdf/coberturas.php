<?php
	...
	if($categoria==0){
		...
		$pdf->Cell(30,7,'Cuota de pago',1,0,'C',1);

	}else{

		$pdf->Cell(85,7,utf8_decode('Categoría'),1,0,'C',1);
		$pdf->Cell(55,7,utf8_decode('Descripción'),1,0,'C',1);
		$pdf->Cell(25,7,utf8_decode('Valor asegurado'),1,0,'C',1);
		$pdf->Cell(15,7,utf8_decode('Tasa'),1,0,'C',1);
	...
	for($i=0;$i<count($this->tpl->coberturas);$i++){
		...
		if($categoria==0){	
			...
			$pdf->Cell(20,6,$this->tpl->coberturas[$i]->fec_vto,'LR',0,'C',$fill);
			$pdf->Cell(30,6,"$ ".number_format($this->tpl->coberturas[$i]->imp_prima,2 ,'.', ','),'LR',0,'R',$fill);
		
		}else{

			$pdf->Cell(85,6,utf8_decode($this->tpl->coberturas[$i]->txt_desc_categ),'LR',0,'L',$fill);//60
			$pdf->Cell(55,6,$this->tpl->coberturas[$i]->txt_desc2,'LR',0,'L',$fill);//60
			$pdf->Cell(25,6,"$ ".number_format($this->tpl->coberturas[$i]->imp_suma_aseg,2 ,'.', ','),'LR',0,'R',$fill);//40
			$pdf->Cell(15,6,number_format($this->tpl->coberturas[$i]->pje_tasa,2 ,'.', ',')." %",'LR',0,'R',$fill);//20
