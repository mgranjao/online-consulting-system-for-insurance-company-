<?php
	...
	if($categoria==0){
            ...
            for($i=0;$i<count($this->tpl->coberturas);$i++){
                ...
                $aux[$this->tpl->coberturas[$i]->cod_ind_categ]=$this->tpl->coberturas[$i]->txt_desc_categ;
            ...
            foreach ($aux as $key => $value){
                ...
                if($this->tpl->mostrarValorAsegurado==TRUE)
                {
                    $pdf->Cell(190,6,utf8_decode($value),'LR',0,'L',TRUE);
                }else{
                    $pdf->Cell(160,6,utf8_decode($value),'LR',0,'L',TRUE);
                }
                $pdf->Ln();
                $pdf->SetFillColor(224, 235, 255);
                $pdf->SetTextColor(0);
                $pdf->SetFont('');
                for($i=0;$i<count($this->tpl->coberturas);$i++){
                    if($this->tpl->coberturas[$i]->cod_ind_categ==$key)
                    {
                        if($i%2==0)
                                $fill=true;
                        else
                        ...
                        $pdf->Cell(60,6,utf8_decode($this->tpl->coberturas[$i]->txt_desc_categ),'LR',0,'L',$fill);
                        ...
                        $pdf->Cell(40,6,number_format($this->tpl->coberturas[$i]->pje_tasa,2 ,'.', ',')." %",'LR',0,'R',$fill);
