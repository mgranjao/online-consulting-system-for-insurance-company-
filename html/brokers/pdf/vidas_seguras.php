<?php
		...
		for($i=0;$i<count($this->tpl->vidas_seguras);$i++){
			...
			if($this->tpl->vidas_seguras[$i]->estadoActual=="ACTIVO"){
			
			$cont=$i+1;
			
			$pdf->Cell(5,6,$cont,'LR',0,'L',$fill);
			$pdf->Cell(90,6,utf8_decode(texto($this->tpl->vidas_seguras[$i]->apellido1))." ".utf8_decode(texto($this->tpl->vidas_seguras[$i]->apellido2))." ".utf8_decode(texto($this->tpl->vidas_seguras[$i]->nombre)),'LR',0,'L',$fill);

			$pdf->Cell(30,6,utf8_decode(texto($this->tpl->vidas_seguras[$i]->documento)),'LR',0,'L',$fill);
			$pdf->Cell(30,6,utf8_decode(texto($this->tpl->vidas_seguras[$i]->fechaAlta)),'LR',0,'L',$fill);
		...
		$pdf->Output("equivida-asegurados.pdf",'I');
